...
const PasswordInput = ({ value, onChange,placeholder, name }) => {
  ...
  return (
    <div className='flex items-center justify-between py-2 px-6 border focus-within:border-2 focus-within:border-blue-500 border-gray-400 rounded-sm w-full'>
      <input type={showPassword ? "text" : "password"} name={name ? name : "password"} value={value} onChange={onChange} placeholder={placeholder ? placeholder : "Enter Password"}  className='outline-none '/>
